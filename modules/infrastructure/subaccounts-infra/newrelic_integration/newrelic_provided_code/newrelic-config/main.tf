...
resource "aws_iam_role" "newrelic_aws_role" {
  name               = var.role_name_override == "" ? "NewRelicInfrastructure-Integrations-${var.name}" : var.role_name_override
...
resource "aws_kinesis_firehose_delivery_stream" "newrelic_firehose_log_stream" {
  name        = "newrelic_firehose_log_stream_${var.name}"
  destination = "http_endpoint"
  http_endpoint_configuration {
    url                = var.newrelic_account_region == "US" ? "https://aws-api.newrelic.com/firehose/v1" : "https://aws-api.eu.newrelic.com/firehose/v1"
    name               = "New Relic Logs ${var.name}"
    access_key         = newrelic_api_access_key.newrelic_aws_access_key.key
    buffering_size     = 1
    buffering_interval = 60
    role_arn           = aws_iam_role.firehose_newrelic_role.arn
...
resource "newrelic_cloud_aws_integrations" "newrelic_cloud_integration_pull" {
  ...
  ses {
    aws_regions              = ["us-east-2", "us-east-1"]
    metrics_polling_interval = 300
  }
  sns {
    aws_regions = ["us-east-2"]
