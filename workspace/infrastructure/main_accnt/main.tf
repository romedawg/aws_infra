import {
  to = aws_organizations_organization.rome
  id = "o-z7ap479ljb"
}

resource "aws_organizations_organization" "rome" {
...
## strip this away, was a testing module
module "organization" {
  source = "../../../modules/infrastructure/organization"
}

module "organization2" {
  source = "../../../modules/infrastructure/organization/organization_accounts"
}
...
module "transit_gateway" {
  source                     = "../../../modules/infrastructure/networking/transit_gateway"
  subaccount_tgw_attachments = module.subaccount_tgw_attachments.tgw_attachment_ids
}

module "subaccount_tgw_attachments" {
  source                        = "../../../modules/infrastructure/networking/transit_gateway/attachment_subaccounts_finder"
  managed_organization_accounts = { for key, val in module.organization2.accounts : key => val if val.share_resources }
}

module "ram" {
  source                        = "../../../modules/infrastructure/organization/resource_access_manager"
  managed_organization_accounts = { for key, val in module.organization2.accounts : key => val if val.share_resources }
  // TODO create this via terraform
  managed_prefix_arns        = {}
  subaccount_tgw_attachments = module.subaccount_tgw_attachments.tgw_attachment_ids
  // TODO create this via terraform
  transit_gateway_arn = module.transit_gateway.gohealth_transit_gateway_arn
}
